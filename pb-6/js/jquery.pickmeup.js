...
(function (factory) {
	...
	function fill () {
		...
		for (var i = 0; i < options.calendars; i++) {
          ...
          if(local_date.valueOf() < today) {
						day.class_name.push('pmu-day-lost');
          }
					if (local_date.getDay() == 0) {
						day.class_name.push('pmu-sunday');
					} else if (local_date.getDay() == 6) {
						day.class_name.push('pmu-saturday');
					}else if (local_date.getDay() == 1) {
						day.class_name.push('pmu-monday');
					}
          if(day.text == '1')
						day.class_name.push('pmu-first-day');
	...
	function click (e) {
		...
		if (el.length) {
			if (el.hasClass('pmu-disabled') || el.hasClass('pmu-day-lost')) {
				return false;
			}
			var	$this			= $(this),
				options			= $this.data('pickmeup-options'),
				instance		= el.parents('.pmu-instance'),
        ...
        $('.pmu-days .pmu-selected').first().addClass('first');
        $('.pmu-days .pmu-selected').last().addClass('last');
        $('.pmu-days .pmu-selected:not(.first)').first().addClass('second');
        $('.pmu-days .pmu-selected:not(.last)').last().addClass('pre-last');
        //$('.pmu-days .pmu-not-in-month').next().addClass('circleCorner');
		return false;
	}
